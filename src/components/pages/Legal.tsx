  ...
  zh: {
    ...
    sections: [
      ...
      { id: 'cgu', title: '2. 用户条款', tocLabel: '用户条款', blocks: [
        ...
        { heading: '责任', content: '应用按“现状”提供。滑板存在风险：你需自行承担滑行的责任。' },
  ...
  ja: {
    ...
    sections: [
      ...
      { id: 'cgu', title: '2. 利用規約', tocLabel: '利用規約', blocks: [
        ...
        { heading: '責任', content: 'アプリは「現状のまま」提供されます。スケートには危険が伴います。滑走は自己責任です。' },
...
export function Legal({ section }: { section?: string }) {
  ...
  return (
    <main className="page">
      ...
      <p className="page__sub">{data.updated}</p>

      <nav className="page__toc" aria-label={data.title}>
        {data.sections.map((s) => (
          <a key={s.id} href={`#page=legal:${s.id}`}>{s.tocLabel}</a>
        ))}
      </nav>
        ...
        <section className="legal__section" id={s.id} key={s.id}>
          ...
          {s.blocks.map((b, i) => (
            <div key={i}>
              ...
              <p className="page__p">{b.content}</p>
            </div>
          ))}
        </section>
      ))}
