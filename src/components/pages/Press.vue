...
const c = computed(() => PRESS[lang.value])
</script>
...
<template>
  <main class="page">
    ...
    <h2 class="page__h2">{{ c.taglineHeading }}</h2>
    <p class="page__p">{{ c.tagline }}</p>

    <h2 class="page__h2">{{ c.factsHeading }}</h2>
    <ul class="page__list">
      <li v-for="f in c.facts" :key="f">{{ f }}</li>
    </ul>

    <h2 class="page__h2">{{ c.logoHeading }}</h2>
    <a class="page__asset" :href="logo" download>
      <img :src="logo" alt="bmx" />
      {{ c.logoLabel }}
    </a>

    <h2 class="page__h2">{{ c.founderHeading }}</h2>
    <p class="page__p">{{ c.founder }}</p>

    <h2 class="page__h2">{{ c.contactHeading }}</h2>
    <p class="page__p">
      {{ c.contactIntro }}<br />
      <a class="page__mail" href="mailto:press@bmx.bike">press@bmx.bike</a>
    </p>
  </main>
</template>
