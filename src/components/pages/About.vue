...
const ABOUT: Record<Lang, Content> = {
  ...
  zh: {
    title: '关于',
    subtitle: 'bmx 是没有竞争、没有自我的BMX——一个团队，每一条线路、每一次摔倒、每一个点位都重要。',
    cards: [
      { title: '我们的使命', text: '为BMX本来的样子喝彩：一种文化、一个社区、一种自由。没有排名，没有压力——只有骑行的冲动。' },
      { title: '如何运作', text: '发布你的片段，把点位标在地图上，为你喜欢的车手投票，和团队聊天。全部免费。' },
      { title: '社区', text: '没有算法，没有用点赞衡量你的价值。只有车手分享他们所热爱的，遍布世界。' },
      { title: '没有压力', text: '在这里，我们为尝试喝彩，不亚于为成功的招式。按你的节奏滑，为自己进步。' },
    ],
  },
  ja: {
    title: '概要',
    subtitle: 'bmx は競争もエゴもないBMX——すべてのライン、転倒、スポットが大切なクルー。',
    cards: [
      { title: '私たちの使命', text: 'BMXをありのままに称える：文化、コミュニティ、自由。ランキングもプレッシャーもなく、ただ滑りたい気持ちだけ。' },
      { title: '使い方', text: 'パートを投稿し、スポットを地図に置き、好きなライダーに投票し、クルーと話そう。すべて無料。' },
      { title: 'コミュニティ', text: 'アルゴリズムもなく、価値を決めるいいねもない。ただ好きなものを分かち合うライダーが世界中にいるだけ。' },
...
const c = computed(() => ABOUT[lang.value])
</script>
...
<template>
  <main class="page">
    ...
    <p class="page__sub">{{ c.subtitle }}</p>
    <div class="page__grid">
      <article class="card" v-for="card in c.cards" :key="card.title">
